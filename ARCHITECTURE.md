...
core/: Contiene código compartido entre diferentes features, como manejo de errores, conectividad de red, utilidades y widgets reutilizables.
features/: Organiza el código por funcionalidades. Cada feature tiene su propia implementación de las capas de Clean Architecture.
feature_a/ y feature_b/: Son ejemplos de cómo se estructuraría una feature individual. Reemplaza feature_a y feature_b con nombres descriptivos de las funcionalidades de tu aplicación (ej: auth, home, profile).
data/: Implementa la capa de datos, incluyendo fuentes de datos (remotas y locales), modelos de datos y la implementación de los repositorios.
domain/: Contiene la lógica de negocio, las entidades (objetos del dominio), los interfaces de los repositorios y los casos de uso (acciones que se pueden realizar en el dominio).
presentation/: Contiene la lógica de la interfaz de usuario, incluyendo Blocs (o Cubits) para el manejo del estado, las páginas (pantallas) y widgets específicos de la feature.
